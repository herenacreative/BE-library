...
module.exports = {
  getAllBorrow: async function (req, res) {
    ...
    try {
      if (!search && !sort) {
        ...
        const result = await borrowModel.searchPageSortModel(
          "",
          sort,
          page,
          limit
  ...
  getPendingBorrowByUserId: async function (req, res) {
    const user_id = req.query.user_id;
    const book_id = req.query.book_id;
    try {
      const result = await borrowModel.getPendingByUserIdModel(
        user_id,
        book_id
      );
      return helpers.response(res, "success", result, 200);
  ...
  postBorrow: async function (req, res) {
    ...
    try {
      ...
      //   const insertId = result.id.insertId;
      //   const getBorrowData = await borrowModel.getBorrowModelById(insertId);
      //   console.log(getBorrowData);
      //   const result = await borrowModel.postReturnedModel(setData);
      return helpers.response(res, "Book Borrow", result, 200);
      // }
      // return helpers.response(res, 'book Not Available', 400)
      // try {
  ...
  getAllReturn: async function (req, res) {
    ...
    try {
      if (!search && !sort) {
        ...
        const result = await returnModel.returnSearchPageSortModel(
          "",
          sort,
          page,
          limit
